...
>
> **Ye file ab bhari hui hai — hafta band ho gaya (`2026-08-29`).** Chhe entries (Din 1–6) plus Stage Day,
> plus week close. Jahan bhi `____` bacha hai wo template ki apni line hai, missing measurement nahi.
> Original rule, record ke liye: har `____` uss din shaam bharti thi, aur jo value measure nahi hui uske
> liye `[NO EVIDENCE]` likha jaata tha — blank ko guess se bharna sabse mehnga shortcut hai.
>
...
- [Din 1 — Problem statement, phir lease column](#din-1--problem-statement-phir-lease-column-2026-08-23) · `partial`
- [Din 2 — Reaper: recovery bahar se aati hai](#din-2--reaper-recovery-bahar-se-aati-hai-2026-08-25) · `partial`
- [Din 3 — 🎯 Zinda par slow worker: ek job, do execution](#din-3--zinda-par-slow-worker-ek-job-do-execution-2026-08-26) · `yes`
- [Din 4 — Bounded retry, backoff, jitter](#din-4--bounded-retry-backoff-jitter-2026-08-27) · `partial`
- [Din 5 — `dead_letter` + graceful shutdown](#din-5--dead_letter--graceful-shutdown-2026-08-28) · `partial`
- [Din 6 — Close: reconcile, likho, handoff](#din-6--close-reconcile-likho-handoff-2026-08-29) · `yes`
- [Week close — reconcile chain aur handoff](#week-close--reconcile-chain-aur-handoff)

> **Hafta band:** lease · reaper · heartbeat · bounded retry (backoff + equal jitter) · `dead_letter`.
> Chain paanch status buckets par exactly judi (`107` jobs, `94` execution rows). **Contract #2 abhi bhi
> unprotected hai — duplicate window narrow hui, band nahi hui.** Handoff:
> [`../daily/WEEK_02_HANDOFF.md`](../daily/WEEK_02_HANDOFF.md).
...
hai ki 41/63/65 ka evidence dobara reproducible **nahi** hai — **aur exactly wahi hua** `[MEASURED-R]`:
Din 2 ne teeno ko `running → pending` reclaim kiya aur Din 3 ne teeno ko drain kar diya. **Aaj teeno
`succeeded` hain aur `attempts = 0`** par baithi hain. Unka pre-reclaim state (`running` + `claimed_at NULL`
+ ek hafta purana `created_at`, aur `41` ki `job_executions` me **koi** row nahi) sirf **Din 2 ki entry ke
verbatim dump me** zinda hai — database se wo wapas nahi aa sakta. Din 3 aage se apni stuck rows khud seed
karta raha (`91`, phir Din 4/5 ke reviewer probes), aur wahi sahi tha.
