...
- [Day 1 — Schema design + Week 0 ka udhaar (2026-08-14)](#day-1--schema-design--week-0-ka-udhaar-2026-08-14)
- [Day 2 — API: `POST /jobs` and `GET /jobs/{id}` (2026-08-15)](#day-2--api-post-jobs-and-get-jobsid-2026-08-15)
- [Day 3 — Worker loop: `claim → execute → mark` (2026-08-16)](#day-3--worker-loop-claim--execute--mark-2026-08-16)
- [Day 4 — Two workers, one job: build the instrument, then break the queue (2026-08-17)](#day-4--two-workers-one-job-build-the-instrument-then-break-the-queue-2026-08-17)
- [Day 5 — `kill -9` mid-job: make a job disappear, then prove nobody can find it (2026-08-18)](#day-5--kill--9-mid-job-make-a-job-disappear-then-prove-nobody-can-find-it-2026-08-18)


---

## Day 1 — Schema design + Week 0 ka udhaar (2026-08-14)
...
**Goal met?** Yes for the table and migration. Partially for Part A — three of four items were already closed in Week 0's log; the fourth (Cloudflare postmortem) is deliberately still open.
...
### 🧠 Self-Check (honest — 0 / 9 self-answered)

**The six schema decisions and the three additions were explained to me, not derived by me.** I read the trade-offs for the first time today and understood roughly 60% on the first pass. I asked for the answers rather than attempting them, so **nothing here counts as self-answered**, and the `DECISIONS.md` entries carry a provenance note saying so.
...
**The pattern to be honest about:** this is the third day running where the measurements are solid and the concept-formation is the weak link — Day 4 was 3/8 self-answered, Day 5 was 2.5/10, today 0/9. Today is worse than it looks, because I did not attempt the questions at all before asking for answers. Everything above is currently **recognisable, not recallable.**

One thing genuinely mine: choosing the sync driver for Alembic and async for the app, which avoided the `env.py` problem the plan warned about.

**Weekend consolidation must re-test all nine cold.** Highest priority, because it has now failed twice under different framings: *why row-level detection misses write skew* (Day 5 Q4, then Q2b).
...
**Deliberately open:**
- **`POSTMORTEMS.md` entry #2 (Cloudflare)** — the only Din 1 checklist item not done, left open by choice. Week 0's Definition of Done wants 2 entries and there is currently **1**. *(A duplicate Buttondown entry was removed today — it had been written twice, so the count looked like 2 when it was 1.)*
- **Claim query index** — `P-03`. To be **measured** with `EXPLAIN ANALYZE` in Week 4, not guessed now.
- **`idle_in_transaction_session_timeout`** — new after `P-06`. Postgres will not clean up after a forgetful client. Belongs in `DECISIONS.md` alongside pool sizing in Week 4; both are unowned defaults in the same layer.

**Not measured, still inference:**
- **The `ACCESS EXCLUSIVE` lock-queue hazard** in `D-07`. M5 proves no table rewrite, but says nothing about lock waits, and there was no contention during the test. Would need: hold a long transaction on the table, then time the `ALTER`.
- **Age of the abandoned transactions in `P-06`.** The container was created 5 days ago but reports "Up 2 days", so the sessions cannot be as old as the container. `xact_start` would have settled it and was not captured before terminating them. **"Five days" is an upper bound from container age, not a measured transaction age.**

**Carried over from Week 0, still open:**
- **Day 4 Exp B's original goal** — the fast-fail (RST → instant `ConnectError`) versus slow-fail (blackhole → full timeout) contrast has **never been reproduced** on this machine. A raw socket to a closed port also timed out at 1.001s, which correctly ruled out httpx as the cause. **The cause itself is not identified** — Windows Firewall, an AV network filter, and Docker Desktop networking are all unverified guesses. Worth retrying on Linux/WSL.
- **Day 3 Exp D** — `pg_stat_activity` connection count during pool load, never recorded. *(Tooling is clearly not the blocker: `pg_stat_activity` was used twice today.)*
- **Day 2 grace period** — exit code `137` where the arithmetic predicted `0`, still unexplained. Needs `Measure-Command { docker stop ... }`.

---

### ❓ Question / Next Thought

Din 2 builds `POST /jobs`, and today's `id` decision sharpened the question waiting there. `D-03` deliberately kept `id` internal and DB-generated, on the grounds that a primary key and an idempotency key are different concerns. That leaves the duplicate-POST problem entirely unsolved for now: response lost in the network → client retries → two rows, both legitimate-looking, with nothing linking them.

So the Din 2 question is not "how do I stop that" — it is **what does `202 Accepted` actually promise when the caller may never learn that it was accepted?** Relay's contract says an accepted job is never lost. It says nothing about a job the client does not know was accepted. That gap is exactly where Week 3 lives, and Din 2 should make it concrete rather than close it.

Second, smaller: today's `status` decision means the claim's safety rests entirely on my discipline in never writing a `status` update without a `WHERE` guard on the old value. The database will not remind me. Is that something a test can enforce, or only a convention? *(Din 3.)*
